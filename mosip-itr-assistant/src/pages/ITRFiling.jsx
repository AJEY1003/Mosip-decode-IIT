...
const ITRFilingPage = () => {
    ...
    const handleQRUpload = async (file) => {
        ...
        try {
            console.log('🔍 Processing QR file:', file.name, file.size, 'bytes');
            ...
            // Call backend API to decode QR
            console.log('🚀 Calling backend API...');
            const decoded = await apiService.decodeQRData(base64Data);
            console.log('📦 Backend response:', decoded);
            
            if (decoded.status === 'success') {
                const formSections = decoded.form_sections;
                console.log('📋 Form sections received:', formSections);
                ...
                setItrFormData(newFormData);
                
                setQrProcessed(true);
                console.log('✅ QR Code processed and form auto-filled');
            } else {
                console.error('❌ Backend returned error:', decoded.message);
                setError('Failed to decode QR code data: ' + decoded.message);
            }
        } catch (error) {
    ...
    // Handle test QR with sample data
    const handleTestQR = async () => {
        setLoading(true);
        setError(null);

        try {
            console.log('🧪 Generating test QR data...');
            
            // Get test QR from backend
            const testQR = await apiService.generateTestITRQR();
            console.log('📦 Test QR response:', testQR);
            
            if (testQR.qr_data) {
                // Use the JSON string directly
                const decoded = await apiService.decodeQRData(testQR.qr_data);
                ...
                if (decoded.status === 'success') {
                    ...
                    setQrProcessed(true);
                    console.log('✅ Test QR processed and form auto-filled');
                } else {
                    console.error('❌ Backend returned error:', decoded.message);
                    setError('Failed to decode test QR data: ' + decoded.message);
                }
            } else {
                setError('Failed to generate test QR data');
            }
        } catch (error) {
            console.error('❌ Test QR error:', error);
            setError('Failed to process test QR data.');
        } finally {
            setLoading(false);
        }
    };
